...
public class GuiQualitySettingsOF extends GuiScreen
{
    ...
    public void drawScreen(int i, int j, float f)
    {
        ...
        if (guibutton != null)
        {
            ...
            for (int l1 = 0; l1 < as.length; l1++)
            {
                ...
                fontRenderer.drawStringWithShadow(s1, l + 5, i1 + 5 + l1 * 11, i2);
            }
        }
    }
    ...
    private String[] getTooltipLines(String s)
    {
        ...
        if (s.equals("Anisotropic Filtering"))
        {
            return (new String[]
                    {
                        "Anisotropic Filtering", " OFF - (default) standard texture detail (faster)", " 2-16 - finer details in mipmapped textures (slower)", "The Anisotropic Filtering restores details in mipmapped", "textures. Higher values may decrease the FPS."
                    });
        }

        if (s.equals("Antialiasing"))
        {
            return (new String[]
                    {
                        "Antialiasing", " OFF - (default) no antialiasing (faster)", " 2-16 - antialiased lines and edges (slower)", "The Antialiasing smooths jagged lines and ", "sharp color transitions.", "Higher values may substantially decrease the FPS.", "Not all levels are supported by all graphics cards.", "Effective after a RESTART!"
                    });
        }
    ...
    static
    {
        enumOptions = (new EnumOptions[]
                {
                    EnumOptions.MIPMAP_LEVEL, EnumOptions.MIPMAP_TYPE, EnumOptions.AF_LEVEL, EnumOptions.AA_LEVEL, EnumOptions.CLEAR_WATER, EnumOptions.RANDOM_MOBS, EnumOptions.BETTER_GRASS, EnumOptions.BETTER_SNOW, EnumOptions.CUSTOM_FONTS, EnumOptions.CUSTOM_COLORS,
                    EnumOptions.SWAMP_COLORS, EnumOptions.SMOOTH_BIOMES, EnumOptions.CONNECTED_TEXTURES
                });
    }
}
